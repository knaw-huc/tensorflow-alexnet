...
kernel = tf.Variable(tf.random.truncated_normal([11,11,3,first_conv_size], dtype=tf.float32,stddev=1e-1), name="conv1_weights")
conv = tf.nn.conv2d(input_images, kernel, [1,4,4,1], padding="SAME")
bias = tf.Variable(tf.random.truncated_normal([first_conv_size]))
...
kernel = tf.Variable(tf.random.truncated_normal([5, 5, first_conv_size, second_conv_size], dtype=tf.float32, stddev=1e-1), name="conv2_weights")
conv = tf.nn.conv2d(pooled_conv1, kernel, [1, 4, 4, 1], padding="SAME")
bias = tf.Variable(tf.random.truncated_normal([second_conv_size]), name="conv2_bias")
conv_with_bias = tf.nn.bias_add(conv, bias)
...
kernel = tf.Variable(tf.random.truncated_normal([3, 3, second_conv_size, third_conv_size], dtype=tf.float32, stddev=1e-1), name="conv3_weights") 
conv = tf.nn.conv2d(pooled_conv2, kernel, [1, 1, 1, 1], padding="SAME")
bias = tf.Variable(tf.random.truncated_normal([third_conv_size]), name="conv3_bias")
conv_with_bias = tf.nn.bias_add(conv, bias)
conv3 = tf.nn.relu(conv_with_bias, name="conv3")

## Fourth convolutional layer
kernel = tf.Variable(tf.random.truncated_normal([3, 3, third_conv_size, fourth_conv_size], dtype=tf.float32, stddev=1e-1), name="conv4_weights") 
conv = tf.nn.conv2d(conv3, kernel, [1, 1, 1, 1], padding="SAME")
bias = tf.Variable(tf.random.truncated_normal([fourth_conv_size]), name="conv4_bias")
conv_with_bias = tf.nn.bias_add(conv, bias)
conv4 = tf.nn.relu(conv_with_bias, name="conv4")

## Fifth convolutional layer
kernel = tf.Variable(tf.random.truncated_normal([3, 3, fourth_conv_size, fifth_conv_size], dtype=tf.float32, stddev=1e-1), name="conv5_weights")
conv = tf.nn.conv2d(conv4, kernel, [1, 1, 1, 1], padding="SAME")
bias = tf.Variable(tf.random.truncated_normal([fifth_conv_size]), name="conv5_bias")
conv_with_bias = tf.nn.bias_add(conv, bias)
